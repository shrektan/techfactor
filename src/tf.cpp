...
void assert_valid(const std::map<RDate, Timeseries>& x, const int ncol)
{
  // ensure all the timeseries is the same length
  std::set<int> res;
  for (auto& pair : x) res.emplace(pair.second.size());
  if (res.size() > 1) Rcpp::stop(
      "The child size in x must be the same."
  );
  if (res.size() == 1 && *res.begin() != ncol) Rcpp::stop(
    "The child size (%d) in x must be the same as ncol(%d)",
    *res.begin(),
    ncol
  );
}
...
Rcpp::NumericMatrix convert_to_mat(
    const std::map<RDate, Timeseries>& x, const int ncol)
{
  assert_valid(x, ncol);
  const int nrow = x.size();
  Rcpp::NumericMatrix res(ncol, nrow);
  auto iter = res.begin();
  for (auto& pair : x) {
    std::copy(pair.second.cbegin(), pair.second.cend(), iter);
    std::advance(iter, ncol);
  }
  return Rcpp::transpose(res);
}
